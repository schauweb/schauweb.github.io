...
var Penguin = function() {
    ...
    p.render = function(ctx, lagOffset) {
        var rX = ((p.visualX - p.oldX) * lagOffset) + p.oldX + PENGUIN_X;
	...
	p.calm = function() {
		p.calmStatus = p.animationType == STILL ? CALMED : SIGNAL_CALM;
	}

	p.isCalmed = function() {
		return p.calmStatus == CALMED;
	}

    p.animate = function() {
        if (p.animationType == STILL ||
			p.animationType == YUCKKING ||
			p.isCalmed()) {
            return;
        }
...
function addPenguin(x, type, face) {
	...
	penguin.init(sheet, x, PENGUIN_Y, type, face);
